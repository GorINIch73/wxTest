...
: wxPanel(parent,wxID_ANY,wxDefaultPosition,wxSize(270,150),wxBORDER_SUNKEN)
{
    ...
    m_TextCtrlSqlQuery = new wxTextCtrl( this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, 0 );
    m_TextCtrlSqlQuery->SetValue("SELECT * from bank");
      // m_SqlQueryTextCtrl->SetToolTip(_("Enter SQL query"));
   
    m_ButtonSqlGo = new wxBitmapButton( this, ID_RUN, wxBitmap(_home_32_xpm), wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW );
    ...
    //m_ListBoxResult= new wxListBox(this,wxID_ANY,wxDefaultPosition,wxDefaultSize);
    ...
    //     m_ListBoxResult->Append(str);
        ...
        Connect(ID_RUN,wxEVT_COMMAND_BUTTON_CLICKED,wxCommandEventHandler(rightPanel::OnRun));
...
void rightPanel::OnRun(wxCommandEvent& event)
{
    // тест загрузки базы
   ...
   wxSQLite3ResultSet Res;
    try 
    {
      ...
      wxMessageOutputDebug().Printf("Open error: %i", database->GetSystemErrorCode());

      wxMessageOutputDebug().Printf("Querry: %s ", m_TextCtrlSqlQuery->GetValue());
      
      database->Begin();
      //database-<query
      //database->PreparePersistentStatement(wxString::Format(_("%s"),m_TextCtrlSqlQuery->GetValue()));
      //Res = database->ExecuteUpdate();
      Res = database->ExecuteQuery(wxString::Format(_("%s"),m_TextCtrlSqlQuery->GetValue()));
    //if (database->GetSystemErrorCode()!=0) wxLogError(wxString(_("Error: "))+wxString::Format(_("%i"),database->GetSystemErrorCode()));
     // wxSQLite3Exception& e0;
      wxMessageOutputDebug().Printf("***** Select %i", database->GetSystemErrorCode());
...
//    m_GridResult->CreateGrid( 0, Res.GetColumnCount() );  //временно
    ...
    if (numCol>0) 
    {
          m_GridResult->CreateGrid( 0, numCol);  //временно - второй раз нельзя - надо найти способ повторной инициализации
        //m_GridResult->CreateGrid( 0, Res.GetColumnCount() );  //временно

        // Label some columns
        while(numCol>0)
        {
            m_GridResult->SetColLabelValue(numCol-1, Res.GetColumnName(numCol-1));
//            wxLogInfo(wxString::Format(_("%s"),Res.GetColumnType(numCol-1)));
            wxMessageOutputDebug().Printf("ColumnType=%i", Res.GetColumnType(numCol-1));
            numCol--;
        }
